<script>
  ...
  function nextImage(e) {
    e.stopPropagation();
    const images = activeProject.images || (activeProject.image ? [activeProject.image] : []);
    if (images.length > 0) {
  ...
  <div class="container">
    <ScrollReveal>
      <div class="portfolio-header">
        <h2 data-content-id="portfolio_headline">{content.portfolio_headline}</h2>
        <p data-content-id="portfolio_subheadline">{content.portfolio_subheadline}</p>
      </div>
    </ScrollReveal>
    ...
    <div class="modal-content card" onclick={(e) => e.stopPropagation()}>
      ...
      {#if activeProject.images?.length > 0 || activeProject.image}
        ...
        <div class="modal-image-container">
          <img src={getImageUrl(images[currentImageIndex])} alt={activeProject.name} class="modal-image" />
          ...
          {#if images.length > 1}
            ...
            <div class="carousel-dots">
              {#each images as _, i}
                <span class="dot" class:active={i === currentImageIndex} onclick={(e) => { e.stopPropagation(); currentImageIndex = i; }}></span>
              {/each}
            </div>
          {/if}
        </div>
